...
def makeSubsets(itemSet):
	...
	return subsetHelper(itemSet, itemSet, maxLength)

def subsetHelper(baseItemset, iterableItemset, max):
	if len(iterableItemset) ==0 or len(iterableItemset[0]) == max-1:
		return list()
	nextLevel = list()
	for item in iterableItemset:
		for addedItem in baseItemset:
			if addedItem not in item:
				newItemSet = ''.join(sorted(item + addedItem))
				if newItemSet not in nextLevel:
					nextLevel.append(newItemSet)
	return nextLevel + subsetHelper(baseItemset, nextLevel, max)
...
outputString = ''
for itemSet in findAllFrequentItemsets(database, frequentItemset, supportThreshold):
	outputString+=','+itemSet
print(sorted(outputString[1:].split(',')))